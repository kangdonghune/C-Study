...
public:
	...
	void Init() //별도의 초기화 함수. 이를 반드시 선언하도록 하면 초기화 빼먹는 걸 방지.
	{
	...
	//보통 생성자가 초기화에 쓰인다면 소멸자는 멤버 동적 변수의 할당 해제에 쓰인다.
	//소멸 시 멤버 변수들 delete하고 nullptr 초기화.

	//클래스를 동적할당을 하게 되면 delete를 해주지 않으면 소멸자가 호출되지 않는다.->메모리 누수.
	//CConstractor Cc = new CConstractor; 같은 경우.
	//동적할당되어 힙에 생성된 클래스는 함수가 종료되도 남아있게 된다.
	~CConstractor()
	{
		cout << "소멸자 호출" << endl;
	}
...
각 헤더에 class 선언만 해주고 정의부인 cpp에 해당 클래스의 헤더를 include하여 해당 클래스의 멤버나 내부 함수 등 활용.
*/
