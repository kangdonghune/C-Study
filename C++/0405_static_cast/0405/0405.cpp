...
int main()
{
	...
	//형변환이 안전하지 않다 판단하면 nullptr로 반환한다. 그러니 다이나믹 캐스트 사용 시 안정성 검사를 실시해보자

	/*
		type1 child = dynamic_cast<child>(parent);
		if(child == nullptr)
		{
			cout << "다이나믹캐스팅에 실패했습니다." << enld;
		}
	child와 parent가 상속관계가 아니거나 형제관계여서 형변환이 불안전하다 판단되면 nullptr이 되고 위처럼 nullptr일 때 조건을 달아주면 문제 발생을 캐치할 수 있다.
